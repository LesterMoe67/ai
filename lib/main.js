...
export function averageMatrix(matrixes) {
    ...
    return new matrixOperator(sum, n => n/matrixes.length)
}
...
export class Model {
       ...
       train(epochs, batchSize, inputs, outputs, learningRate) {
            for (let epoch = 0; epoch < epochs; epoch++) {
                ...
                for (let i = 0; i < inputs.length; i++) {
                    ...
                    // 2. backprop (compute gradients inside layers)
                    ...
                    console.log(
                        ...
                        `loss ${totalLoss / (i + 1)}`,
                        "acc ="+ (Math.abs(100 - Math.sqrt(this.cost(outputs[i]).list[0])/outputs[i][0] * 100)).toFixed(2)+"%"
                    );
                }
            }
        }
    ...
    test(inputSize,func, tests) {
        ...
        for (let i = 0; i < tests; i++) {
            ...
            let output = this.layers[this.layers.length - 1].activation.rows[0][0]
